    ...
    <div role="tabpanel">
        ...
        <div class="tab-content cms">
            <div role="tabpanel" id="main" class="tab-pane active">
                ...
                <?= $form->field($model, 'name')->textInput(['id' => 'article-title', 'maxlength' => 255, /*'style' => 'max-width: 600px;'*/]) ?>

                <?php
                $aliasModel = $model->getAliasModel();
                echo $form->field($aliasModel ?: new MenuTree(), 'alias')
                    ->widget(InputAliasWidget::className(), [
                        'sourceFieldId' => 'article-title',
                ...
                <?= $form->field($model, 'published_date')
                    ...
                    ->hint(
                        ...
                        Yii::t('app', 'Publication date article. The later date and time of publication, the higher is the article in the list.'));
                ?>
                ...
                <?= $form->field($model, 'intro')
                    ...
                    ->hint(
                        Yii::t('app', '<strong>Note:</strong>') . ' ' .
                        Yii::t('app', 'Introductory text is displayed after the article title in the article list. If the field is empty, after the article title are displayed first phrases of the main text.'));
                ?>
            ...
            <div role="tabpanel" id="photo" class="tab-pane">
                ...
                <?php else: ?>
                    ...
                    <?= Yii::t('app', 'Images can be added only after the article will be saved.') ?>
                    <p>&nbsp;</p>
                    <p>&nbsp;</p>
                    <p>&nbsp;</p>
                <?php endif; ?>
            ...
            <div role="tabpanel" id="settings" class="tab-pane">
                ...
                if ($model->isNewRecord) {
                    $model->status = backend\models\Article::STATUS_ACTIVE;
                }
                ?>
                <?= $form->field($model, 'status')
                    ->dropDownList(backend\models\Article::getStatusOptions(), ['style' => 'max-width: 200px;'])
                    ->hint(Yii::t('app', '<strong>Note:</strong>') . ' ' . Yii::t('app', 'Only article with status "Published" is displayed on the site.'));
                ?>
        ...
        <div class="form-group btn-ctrl">
            <?= Html::submitButton(
                ...
                ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary', 'name' => 'saveArticle']
            ) ?>
            <?= Html::submitButton(
                Yii::t('app', 'Apply'),
                ['class' => 'btn btn-default', 'name' => 'applyArticle']
            ); ?>
        </div>
    </div>
    <?php ActiveForm::end(); ?>
