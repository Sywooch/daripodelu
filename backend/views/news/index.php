        ...
        <div role="tabpanel" id="main" class="tab-pane<?php if ($tabIndex === 0) : ?> active<? endif; ?>">
            <?= Button::widget ( [
                ...
                'options' => [
                    ...
                    'id' => 'news-del-btn',
                    'href' => Url::to(['/news/deletescope']),
                    'style' => 'margin:5px',
                ],
                'tagName' => 'a',
            ] ); ?>
            <?php $this->registerJs("
                $('#news-del-btn').on('click', function(e){
                    var keys = $('#newsids').yiiGridView('getSelectedRows');

                    $.ajax({
                        type: 'POST',
                        url: '" . Url::to(['/news/deletescope']) . "',
                        dataType: 'json',
                        data: {ids: keys},
                        beforeSend: function(){
                        ...
                        success: function(data, textStatus, jqXHR){
                            ...
                            $.pjax.reload({container:'#news-gv-container'});
                        },
                        error: function(){
            ...
            <?php Pjax::begin(['id' => 'news-gv-container']); ?>
            <?= GridView::widget([
                ...
                'columns' => [
                    ...
                    [
                        'attribute' => 'status',
                        'filter' => News::getStatusOptions(),
                        'value' => function($model){ return News::getStatusName($model->status); },
