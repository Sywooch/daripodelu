...
namespace rkdev\yii2imagecache;
...
class ImageCache extends \yii\base\Component
{
    ...
    public $filter = ImageInterface::FILTER_UNDEFINED;
...
//        $this->getThumbFullPath(str_replace('/uploads/', '', $path), $size);
    ...
    public function createThumb($srcPath, $dstPath, $size, $mode = ManipulatorInterface::THUMBNAIL_OUTBOUND)
    {
        ...
        {
            $maxSideSize = $this->sizes[$size][0];
            $img = Image::getImagine()->open($srcPath);
            $sourceRatio = $img->getSize()->getWidth() / $img->getSize()->getHeight();
            if ($sourceRatio > 1)
            {
                $ratio = $maxSideSize / $img->getSize()->getWidth();

                $box = new Box($img->getSize()->getWidth(), $img->getSize()->getHeight());
                $scaleBox = $box->scale($ratio);
                $img = $img->resize($scaleBox, $this->filter);

                $thumb = Image::getImagine()->create($scaleBox, new Color('FFF', 100));
                $thumb->paste($img, new Point(0, 0));
            }
            else
            {
                $ratio = $maxSideSize / $img->getSize()->getHeight();
